...
public class player1 : MonoBehaviour
{
    ...
    public GameObject planta;//Prefab that we are going to instantiate with the skill of the player 1.
    ...
    private void Update()
    {
        ...
        if (Input.GetAxisRaw("Horizontal") != 0 || Input.GetAxisRaw("Vertical") != 0)//If we are moving.
        {
        ...
        if(Input.GetAxisRaw("Horizontal") == 0 && Input.GetAxisRaw("Vertical") == 0 && !skillActivada)//If we are moving with activated skill
        {
        ...
        if (Physics.Raycast(ray, out hit, .5f) && Input.GetKey(KeyCode.E)) //If the ray hits something and we are pressing the E button.
        {
            ...
            if (hit.transform.CompareTag("Spot") || hit.transform.CompareTag("Suciedad"))//If the floor has any of those two tags.
            {
                ...
                {
                    Instantiate(planta, hit.transform.position, hit.transform.rotation);//We instantiate our kind of floor.
        ...
        //We store how many and which tiles there are for each player.
        plantados = GameObject.FindGameObjectsWithTag("Planta");
        suciedades = GameObject.FindGameObjectsWithTag("Suciedad");
    }
    ...
    {
        foreach(GameObject obj in plantados)//We instantiate the counterpart of the tile on every tile of a type.
        {
        ...
        foreach (GameObject obj in suciedades)
        {
            Vector3 pos_ = obj.transform.position;
